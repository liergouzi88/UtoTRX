...
class Contract{
  ...
  function __construct($tronApi,$abi,$credential=null){
    ...
    $abi = json_decode(Utils::jsonToArray($abi, 5),true) ;
  ...
  function send()
  {
    ...
    if (isset($this->functions)) {
      ...
      */
      ...
      if(empty($ret->result->result)){ 
          throw new \Exception("调用合约失败：".hex2bin($ret->result->message));
      } 
      if($ret->result->result == false){
        throw new \Exception('建立合约失败·错误的合约.');
      }  
      $signedTx = $this->credential->signTx($ret->transaction);//签名
      $ret = $this->api->broadcastTransaction($signedTx);//广播
      
      if(empty($ret->result)){
          throw new \Exception("交易失败：".hex2bin($ret->message));   
      }  
      return (object)[
        'tx' => $signedTx,
        'result' => $ret->result
